...
const VoteModal = ({
  ...
  const { castVoteWithReason, castVoteWithReasonState } =
    useCastVoteWithReason();
  ...
  const handleVoteStateChange = useCallback((state: TransactionStatus) => {
    switch (state.status) {
      ...
      case "Exception":
        ...
        setErrorMessage(
          getVoteErrorMessage(state?.errorMessage) || <>Please try again.</>
        );
        setIsLoading(false);
        setIsVoteFailed(true);
        break;
      ...
      {isVoteFailed && (
        <div className={classes.transactionStatus}>
          <p className={classes.voteFailureTitle}>
            <>There was an error voting for your account.</>
          </p>
    ...
    <>
      {show && (
        <Modal
          ...
          title={<>Vote on Prop {i18n.number(parseInt(proposalId || "0"))}</>}
